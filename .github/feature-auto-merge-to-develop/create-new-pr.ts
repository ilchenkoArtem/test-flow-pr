...
export const createNewPullRequestByParent = async ({githubToken, parentPullRequest, title}: CreateNewPrArgs) => {
  ...
  const { data: alreadyCreatedPullRequests } = await octokit.rest.pulls.list({
  ...
  const alreadyCreatedPullRequest = alreadyCreatedPullRequests[0];

  if (alreadyCreatedPullRequest) {
    core.warning(`Pull request for ${parentPullRequest.headRef} to ${parentPullRequest.baseRef} already exist. PR: ${alreadyCreatedPullRequest.html_url}`);
    return alreadyCreatedPullRequests[0];
  }
